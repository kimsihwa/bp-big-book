...
dplt_03 %>% 
  ggplot(aes(y = coefficient)) +
    ...
    stat_halfeye(
    aes(xdist = dist_normal(estimate, std_error),
        fill = after_stat(x > 0)
  ),
  normalize = "panels"
  ) +
...
d_emp_plt <- d_emp %>% 
  ...
  mutate(treatment = treatment_effect + control,
         scale_range = case_when(
...
job_employed <- dat_rct_prep$job_employed %>% 
  ...
  mutate(survey = str_extract(survey_name, "Baseline|Program Exit|3 Months"),
...
emp_glm <- glm(formula = `3 Months` ~ assignment, 
               data = job_employed,
               family = "binomial")
...
# If in treatment group, chances of being employed at 3 month is 38% and for control group is about 27%
